...
def extract_embeddings(
    ...
    df["topological_embedding"] = (
        df.drop(columns="id").astype(str).agg(" ".join, axis=1)
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--graph_file", required=True, help="Path to graph object file (.pt)"
    )
    parser.add_argument(
        "--node_ids_file",
        required=True,
    ...
    parser.add_argument(
        "--embedding_output", required=True, help="Output CSV path for embeddings"
    )
    parser.add_argument(
        "--hidden_channels",
        type=int,
