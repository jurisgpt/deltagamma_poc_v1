...
def generate_random_graph(
    num_nodes=100, num_edges=500, num_test=200, output_file="Output_Graph"
):
    """
    ...
    node_features_df = pd.DataFrame(
        node_features.numpy(), columns=["feature_1", "feature_2"]
    )
    node_features_df.insert(0, "id", np.arange(num_nodes))  # Add a column for node IDs
    node_features_df.to_csv(
        "node_features.csv", index=False
    )  # Save node features with IDs to .csv
